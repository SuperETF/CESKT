...
export default function SearchModal({ searchQuery, setSearchQuery, onClose }: Props) {
  return (
      ...
      <div className="bg-white w-full max-w-md rounded-xl shadow-xl">
        <div className="p-4 border-b border-gray-100">
          ...
          {!searchQuery ? (
            ...
            <div className="py-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-bold text-gray-700">검색 결과</h3>
              </div>
              <div className="space-y-3">
                {["트레이너", "운동 종류", "지역"].map((category) => (
                  <div
                    key={category}
                    className="flex items-center gap-3 p-2 hover:bg-gray-50 rounded-lg cursor-pointer"
                  >
                    <i className="fas fa-search text-gray-400" />
                    <span className="text-sm">{searchQuery}와(과) 관련된 {category}</span>
                  </div>
                ))}
              </div>
            </div>
